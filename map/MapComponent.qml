...
Map {
   ...
   MapItemView{
    ...
    function changeViewPortCenter(lat, lon, azimuth) // изменение положения четырехугольника отражающего примерный захват изображения
    {
        ...
        var offsetLat = 0.00034; // ширина "прямоугольника захвата" полученная эксперементальным путем
        var offsetLon = 0.0011; // ширина "прямоугольника захвата" полученная эксперементальным путем
    ...
    function addPopupPoint(){ // функция открывает всплывающее окно с точками
        ...
        var coordinates = map.toCoordinate(Qt.point(map.popupX,map.popupY)); // сохраняем полученные координаты точки
        console.log(coordinates.latitude + " " + coordinates.longitude); // выводим в консоль
        dataBase.createLocalPoint(coordinates.latitude, coordinates.longitude, type); // записываем в консоль
